...
static void gbm_mesa_inode_to_handle(struct bo *bo)
{
	...
	/* A16 minigbm's struct bo has a single union bo_handle (not a per-plane
	 * handles[] array). gbm_mesa buffers share one underlying allocation, so a
	 * single inode-derived handle id is correct. */
		       ...
		       uint64_t use_flags)
{
	...
	struct alloc_args alloc_args = {
		...
		/* RPI4 v3d(render)+vc4(scanout) split-GPU: force LINEAR (modifier 0) so the
		 * buffer is renderable by v3d, scannable by vc4, and importable by Mesa EGL
		 * (eglCreateImageKHR). Tiled modifiers fail mapExternalTextureBuffer import. */
		.force_linear = true,
	...
	if (alloc_args.drm_format == 0) {
		...
		/* A16 drv_bo_from_format added a stride_align arg (use 1 = no extra align). */
