...
// struct backend.name is a non-const char*; use a mutable buffer so we neither
// assign a string literal to char* (-Wwritable-strings) nor cast (-Wcast-qual).
static char gbm_mesa_name[] = "gbm_mesa";
...
struct backend gbm_mesa_backend = {
	...
	// A16 minigbm's struct backend dropped bo_get_map_stride (mapped stride now
	// comes from bo->meta); bo_get_plane_fd was re-added in drv_priv.h.
